...
def insert_bundle_data_for_insertion():
    ...
    for key,value in most_recent_addition.get(*list(most_recent_addition.keys())).items():
        if key.isdigit():
            weight = value.get("grams") * grams_to_kilos
            value.pop('grams')
            net_profit = eval(f"""{value.get("price")} - {value.get("total_discount")} * {value.get("quantity")}""")
    ...
    return {list(most_recent_addition.keys())[0] : calc_net_prof_weight(order_profitability)}
